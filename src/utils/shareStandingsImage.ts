...
function getStatRowHeight(options: {
  labelSize: number;
  valueSize: number;
  valueGap: number;
}) {
  return options.labelSize + options.valueGap + options.valueSize * 0.85;
}
...
function drawYourResultContent(
  ...
  const statCellHeight = getStatRowHeight(statsOptions);
  ...
  const statsStartY =
    footerY - sectionGap - footerFontSize - statsBlockHeight;
  const titleBlockHeight = titleLines.length * titleLineHeight;
  const headerBlockHeight = Math.max(
    titleBlockHeight,
    rallyRankFontSize + previewScale(4),
  );
  const headerBottomY = statsStartY - sectionGap;
  const headerTopY = headerBottomY - headerBlockHeight;
  const rallyRankY =
...
function drawPerformanceStat(
  ...
  ctx.fillText(
    value,
    x + width / 2,
    y + sizes.labelSize + valueGap + sizes.valueSize * 0.85,
  );
  ctx.textAlign = "left";
}
